...
class ImportInvoiceController:
        ...
        def load_employee_data(self):
            try:
                ...
                self.view.from_date.dateChanged.connect(lambda: self.update_to_date_limit(from_date))
                self.view.to_date.dateChanged.connect(lambda: self.update_from_date_limit(to_date))

                if not employee:
                    employee = "Tất cả"
                print("----DỮ LIỆU IMPORT INVOICE TRƯỚC KHI LỌC----")
                print(employee, from_date_str, to_date_str, display, search_term)
                filtered_data = self.query_data.search_import(employee,from_date_str,to_date_str,display,search_term)
